...
Chrome. CI installs the Chromium build pinned by the exact Playwright 1.62.0
lockfile dependency. End-to-end checks include customer success, failure/retry,
open-later/refresh, eligible claims through admin resolution, protected admin
operations, confirmed reset, keyboard WebGL fallback, full commerce/admin
journeys at 360, 390, 430 and 768 pixels, input sizing, important element bounds
and page-level overflow. The preview server never reuses an older running server.

Approved parent-environment remediation evidence on 2026-07-28: `npm run verify`
passed end-to-end. The secret-scanner self-test passed, 84 publishable files
were scanned, both npm audits found 0 vulnerabilities, and lint, typecheck, all
5 Vitest files (160 tests), the production build and `git diff --check` passed.

The full Playwright run completed all 75 project cases across 1440×900, 360×800,
390×844, 430×932 and 768×1024: 29 intended tests passed, 46 intentional
desktop/mobile skips and 0 failed. It included reset-dialog storage-failure
containment in every viewport and the full customer checkout, mock payment,
order, immutable reveal, account and admin journeys at all four mobile/tablet
widths. This evidence does not claim GitHub CI, merge, deployment, reviewer
assignment or a new post-remediation personal-Chrome manual pass.
