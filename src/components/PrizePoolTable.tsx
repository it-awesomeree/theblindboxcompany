...
export function PrizePoolTable() {
  ...
  const prizes = state.series.find((series) => series.status === 'published')?.publishedPrizes ?? PRIZES
          ...
          <tbody>
            {prizes.map((item, index) => (
              <tr key={item.id} className={item.tier === 'Grail' ? 'grail-row' : ''}>
                ...
                <td data-label="Odds">{item.odds}</td>
                <td data-label="Tier"><span className={`tier tier-${item.tier.toLowerCase()}`}>{item.tier}</span></td>
              </tr>
            ))}
          </tbody>
          <tfoot><tr><td /><td>Total — one sealed case</td><td>All ≥ RM100</td><td>10,000</td><td>100%</td><td /></tr></tfoot>
        </table>
      </div>
